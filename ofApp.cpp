...
//    anNode.setup("10.206.231.230");
    ...
    gui.setup();
    gui.add(brightnessSlider1.setup("brightness1", 255, 0, 255));
    gui.add(brightnessSlider2.setup("brightness2", 255, 0, 255));
    gui.add(brightnessSlider3.setup("brightness3", 255, 0, 255));
    gui.add(brightnessSlider4.setup("brightness4", 255, 0, 255));
    gui.add(speedSlider.setup("speed", 100, 0, 100));
    ofSetFrameRate(30);
...
void ofApp::setupSimulation() {
    ...
    gradientColors[3] = ofColor(0, 255, 255);
}

//--------------------------------------------------------------
void ofApp::setBrightnessColors(int trib) {
    int slideVal = 0;
    if (trib == 15) slideVal = brightnessSlider4;
    else if (trib < 7) slideVal = brightnessSlider1;
    else if (trib < 16) slideVal = brightnessSlider2;
    else if (trib < 20) slideVal = brightnessSlider3;
    else slideVal = brightnessSlider4;
    gradientColors[0] = ofColor(0);
    gradientColors[1] = ofColor(0);
    gradientColors[2] = ofColor::fromHsb(ofColor(0, 255, 0).getHue(), 255, slideVal);
    gradientColors[3] = ofColor::fromHsb(ofColor(0, 255, 255).getHue(), 255, slideVal);
}
//--------------------------------------------------------------
void ofApp::updateSimulation() {
    if (ofGetElapsedTimeMillis() < lastChecked) lastChecked = 0;
//    if (ofGetElapsedTimeMillis() - lastChecked > packetTime) {
        for (int i = 0; i < tributaries.size(); i++) {
            tributaries[i].updatePacket(100-speedSlider);
        }
